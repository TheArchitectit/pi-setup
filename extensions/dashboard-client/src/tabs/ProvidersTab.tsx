        ...
        {provider.models.map((m) => {
          ...
          return (
            <tr key={m.id}>
              <td className="mono">
                {m.name || m.id}
                {isDefault && <span className="badge ok" style={{ marginLeft: "0.5rem" }}>default</span>}
              </td>
              <td className="mono">{fmtK(m.contextWindow)}</td>
              <td className="mono">{fmtK(m.maxTokens)}</td>
              <td>
                <span className={`badge ${m.reasoning ? "ok" : ""}`}>{m.reasoning ? "yes" : "no"}</span>
              </td>
...
export function ProvidersTab(): React.ReactElement {
  ...
  if (loading && !data) return <div className="loading-spinner">Loading providers…</div>;
  if (error && !data) {
    return (
      <div className="error-fallback">
        <h2>Failed to load providers</h2>
        <p>{error.message}</p>
      </div>
    );
  }
  ...
  if (providers.length === 0) {
    return (
      <div className="card">
        <h2>Providers</h2>
        <p className="muted">No providers configured. Run <span className="mono">/setup</span> in pi to add one.</p>
      </div>
    );
  }
      ...
      {providers.map(([name, provider]) => {
        ...
        return (
          <section key={name} className="card">
            ...
            {isOpen && <ModelsTable provider={provider} defaultModel={defaultModel} />}
          </section>
        );
      })}
    </div>
