...
describe("readAuth", () => {
  test("reports key presence without exposing the key", () => {
    ...
    assert.ok(!json.includes("sk-secret-value"), "key value must not be serialized");
  });
...
describe("readSnapshot", () => {
  test("assembles providers + settings + auth + links + serverVersion", () => {
    ...
    const links = [
      { name: "sibling", url: "http://localhost:9320", port: 9320, kind: "mega-compact", alive: true },
    ];
    const snap = readSnapshot(join(piDir, "snapshot.json"), "9.9.9", links, piDir);
