...
export function handleVersion(
  ...
  if (req.method !== "GET" || !isEndpoint(req.url ?? "/", ENDPOINTS.version.path)) {
    return false;
  }
...
export function handleHealth(
  ...
  if (req.method !== "GET" || !isEndpoint(req.url ?? "/", ENDPOINTS.health.path)) {
    return false;
  }
...
export async function handleSnapshot(
  ...
  if (req.method !== "GET" || !isEndpoint(req.url ?? "/", ENDPOINTS.snapshot.path)) {
    return false;
  }
...
export function handleLinks(
  ...
  if (req.method !== "GET" || !isEndpoint(req.url ?? "/", ENDPOINTS.links.path)) {
    return false;
  }
  // guardrails-allow PREVENT-PI-004: probes sibling dashboard servers on localhost ports
  detectSiblingDashboards()
    .then((links: DashboardLink[]) => sendJson(res, 200, { links } satisfies LinksResponse))
 ...
 * path-traversal protection + SPA index.html fallback internally.
 */
export function handleStatic(
  ...
  const url = req.url ?? "/";
  if (ctx.serveClientAsset(url, res)) { // guardrails-allow PREVENT-PI-004
    return true;
  }
